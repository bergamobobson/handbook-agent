...
class LashEvaluator:
    ...
    def collect(self) -> pd.DataFrame:
        ...
        for i, tc in enumerate(self.test_cases):
            ...
            console.print(f"         latency=[bold]{latency:.2f}s[/]  L_score={l_score:.2f}")
            console.print(f"         answer : {answer[:80]}{'...' if len(answer) > 80 else ''}")
    ...
    def _run_evaluation(self) -> tuple[dict, float, bool]:
        if self.df.empty:
            raise ValueError("DataFrame vide — appeler collect() avant _run_evaluation()")
        ...
        console.print(f"  Scorers : {[s.name if hasattr(s, 'name') else str(s) for s in ALL_SCORERS]}")
        ...
        with mlflow.start_run(run_name=run_name) as run:
            results = mlflow.genai.evaluate(
                data=[
                    ...
                    for r in self.df[["inputs", "expectations"]].to_dict(orient="records")
                ],
            ...
            for dim, weight, score, threshold in dims:
                ...
                console.print(f"  {dim:<6} {weight:<8.2f} {score:<8.3f} ≥ {threshold:<10.2f} {icon}")
