...
from evaluation.graph.nodes import CLASSIFY_CASES, GRADE_CASES, RETRIEVE_CASES, ROUTING_CASES
...
class GraphEvaluator:
    ...
    def _eval_classify(self) -> pd.DataFrame:
        ...
        for i, tc in enumerate(CLASSIFY_CASES):
            ...
            console.print(f"  {'✅' if passed else '❌'} [{actual_intent:<16}] {tc['input'][:55]}")
            if not passed:
    ...
    def _eval_retrieve(self) -> pd.DataFrame:
        ...
        for i, tc in enumerate(RETRIEVE_CASES):
            ...
            all_content = " ".join(
                doc.page_content.lower() if isinstance(doc, Document) else str(doc).lower()
                for doc in documents
            )
            keywords_found = [kw for kw in tc["relevant_keywords"] if kw.lower() in all_content]
            passed = len(keywords_found) > 0

            console.print(f"  {'✅' if passed else '❌'} {n_docs} docs  keywords={keywords_found or '∅'}  {tc['input'][:45]}")
            rows.append({
                "input": tc["input"],
    ...
    def _eval_grade(self) -> pd.DataFrame:
        ...
        for i, tc in enumerate(GRADE_CASES):
            ...
            console.print(f"  {'✅' if passed else '❌'} relevant={str(actual):<6} (attendu={expected})  {doc_preview}")
    ...
    def run_graph_evaluation(self) -> dict:
        ...
        with mlflow.start_run(run_name=f"GraphEval_{datetime.now().strftime('%Y%m%d_%H%M%S')}") as run:
            ...
            graph_score = (acc_classify + acc_retrieve + acc_grade + acc_routing + structure_ok) / 5
            ...
            console.print(f"  {'─'*35}")
            console.print(f"  {'GRAPH GLOBAL':<20} {graph_score:.1%}   {'✅' if graph_score >= 0.8 else '❌'}")
